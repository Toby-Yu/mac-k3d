...
**Users:** download a Release binary and run `mac-k3d setup`. The wizard installs Docker (and k3d/kubectl/helm/Java) when you choose **Install**. See [binary-initializer-new-machine.md](binary-initializer-new-machine.md).
...
**Users:** download a GitHub Release binary (`mac-k3d-{os}-{arch}`), `chmod +x`, run it in Terminal (`mac-k3d setup`). See [binary-initializer-new-machine.md](binary-initializer-new-machine.md). Rust is not required.
